...
# Plot the 2-d embedding generated by t-SNE.
plot_tsne <- function (tsne) {
  pdat        <- as.data.frame(tsne$Y)
  names(pdat) <- c("d1","d2")
  return(ggplot(pdat,aes(x = d1,y = d2)) +
...
# proportions.
plot_tsne_with_topics <- function (tsne, L) {
...
plot_pca <- function (pca, L) {
  pdat <- cbind(pca$x,data.frame(k = factor(apply(L,1,which.max))))
  return(ggplot(pdat,aes(x = PC1,y = PC2,fill = k)) +
    geom_point(shape = 21,color = "white",size = 2,show.legend = FALSE) +
    scale_fill_manual(values = c("dodgerblue","darkorange","darkblue")) +
    theme_cowplot(font_size = 12))
}
