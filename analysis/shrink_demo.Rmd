...
Here we illustrate the use of adaptive shrinkage in differential
expression analysis using a topic model. Since the effect of the
shrinkage is hard to see on the full data set, we perform differential
expression analysis on a random subset of 100 cells.
...
The adaptive shrinkage is applied separately to each topic. In the
plots, we illustrate the effect of the adaptive shrinkage on topic 3,
which corresponds to B cells.

These two scatterplots show the "raw" *z*-scores and the posterior
("shrunk") *z*-scores, coloured by their average expression. A handful
of genes have much larger *z*-scores than the others, so the
right-hand plot shows the same results but zoomed in on smaller
*z*-scores.
...
This is what the volcano plots look like for topic 3, with and without
adaptive shrinkage:
