...
Since most of the LFCs are zero, the adaptiev shrinkage strongly
...
we restrict our attention to LFCs estimated to be positive. (The Y
axis in the right-hand plot has been cut off at 2,000 genes to give
more detail to the distribution of the true positives.)

```{r kl-vs-fasttopics-1, fig.width=8, fig.height=2.5}
D     <- fastTopics:::min_kl_poisson(fit$F)
i     <- which(de$est > 0)
pdat1 <- data.frame(kl = pmin(D[i],1e-6),de = factor(nonzero_lfc[i]))
pdat2 <- data.frame(z = clamp(de$z[i],-4,+4),de = factor(nonzero_lfc[i]))
p7 <- ggplot(pdat1,aes(x = kl,color = de,fill = de)) +
  geom_histogram(bins = 64) +
  scale_color_manual(values = c("darkorange","darkblue")) +
  scale_fill_manual(values = c("darkorange","darkblue")) +
  scale_x_continuous(breaks = seq(0,1e-6,length.out = 3)) +
