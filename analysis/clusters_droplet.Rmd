...
```{r}
p7 <- pca_plot(poisson2multinom(fit),pcs = 1:2,fill = samples$tissue) +
      labs(fill = "tissue")
plot_grid(p1,p4,p7,nrow = 1,rel_widths = c(1,1,1.5))
```
...
p16 <- pca_plot(poisson2multinom(fit),k = 2)
p17 <- pca_plot(poisson2multinom(fit),k = 6)
p18 <- pca_plot(poisson2multinom(fit),fill = log10(counts[,"Chga"])) +
       labs(fill = "log10(count)",title = "Chga")
p19 <- pca_plot(poisson2multinom(fit),fill = log10(counts[,"Gnat3"])) +
