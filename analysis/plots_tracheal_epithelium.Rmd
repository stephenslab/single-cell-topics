...
Like the droplet data, we also pick up rare and abundant topics in the
pulse-seq data:

```{r abundance-plot-pulseseq, fig.width=4, fig.height=2}
p6 <- create_abundance_plot(fit_pulseseq)
print(p6)
```

Following the steps taken for the droplet data, next we compute PCs,
and inspect the projection of the samples onto PCs to identify
clusters.
...
```{r pulseseq-clustering-2, fig.width=4.5, fig.height=4}
n   <- nrow(pca)
x   <- rep("C",n)
pc3 <- pca[,"PC3"]
pc4 <- pca[,"PC4"]
...
```{r pulseseq-clustering-3, fig.width=4.5, fig.height=4}
...
```{r pulseseq-clustering-4, fig.width=4.5, fig.height=4}
