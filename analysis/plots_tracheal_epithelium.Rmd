...
programs in these data sets. In particular, we compare differential
expression in clusters vs. topics.
...
It can also be helpful to view the cluster assignments projected onto
the principal components of the topic proportions:

<img src="figure/clusters_droplet.Rmd/clustering-8-1.png" width=720
style="display: block; margin: auto">

These clusters closely correspond to cell types. To illustrate this
point, here we layer the PCA plots with expression of
cell-type-specific genes such as *Cdhr3* for ciliated cells.

```{r pca-plots-with-expression-droplet, fig.width=7.5, fig.height=5}
...
fit1  <- select_loadings(poisson2multinom(fit_droplet),loadings = rows1)
...
<img src="figure/clusters_pulseseq.Rmd/structure-plot-1.png" width=1920
style="display: block; margin: auto;">

It can also be helpful to view the cluster assignments projected onto
the principal components of the topic proportions:

<img src="figure/clusters_pulseseq.Rmd/clustering-10-1.png" width=1920
style="display: block; margin: auto;">
...
```{r pca-plots-with-expression-pulseseq, fig.width=7.5, fig.height=5}
...
neuroendocrine cells. The fitted topic models do not separate these
two rare cell types; we identify these cell types as a single cluster.

```{r volcano-plot-droplet-T+N-1, fig.width=4.5, fig.height=4}
...
Although we do not identify clusters for tuft and neuroendocrine
cells, in the droplet data topics 2 and 6 capture variation in gene
expression that corresponds to these cell-types. In particular, topic
$k = 2$ in the "T+N" cluster is enriched for neuroendocrine-specific
genes (e.g., *Chga*) and topic $k = 6$ is enriched for tuft-specific
genes (e.g., *Gnat3*).

```{r volcano-plot-droplet-T+N-2, fig.width=8, fig.height=3, warning=FALSE}
...
Similarly, in the pulse-seq data, in topic $k = 2$ we observe
increased expression of neuroendocrine-specific genes and
underexpression of tuft-specific genes.

```{r volcano-plot-pulseseq-T+N-2, fig.width=4.5, fig.height=4, warning=FALSE}
...
The fact that the topic proportions in tuft and neuroendocrine cells
does not vary greatly suggests relatively small differences in gene
