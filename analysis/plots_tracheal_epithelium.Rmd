...
p11 <- zscores_scatterplot(diff_count_clusters_droplet,
                           diff_count_droplet,"B","k2",
                           c(basal_genes,tuft_neuroendocrine_genes),
                           zmax = 400,xlab = "cluster B",ylab = "topic 2")
p12 <- beta_scatterplot(diff_count_clusters_droplet,diff_count_droplet,
                        "B","k2",c(basal_genes,tuft_neuroendocrine_genes),
						zmin = 2,xlab = "cluster B",y = "topic 2")
plot_grid(p11,p12)
```

Some of the smaller (negative) gene enrichments in topic 2 are driven
by the small number of tuft and neuroendocrine cells ("T+N" cluster)
which are partially modeled by this topic. But because these cells are
dominated by basal cells, these enrichments do not show up as strongly
in the volcano and scatterplots above.
...
```{r pca-cell-cycle-pulseseq-proliferating, fig.width=4, fig.height=3}
...
mixtures of gene programs would better characterize hillock cells, and
indeed topic 4, which is the greatest contributor to this cluster of
hillock cells, picks up a more distinct enrichment signal for hillock
genes:
...
Since this topic also shows up in proliferating cells, we also obtain
strong enrichment of cell-cycle genes.
