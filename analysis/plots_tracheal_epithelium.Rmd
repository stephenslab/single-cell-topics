...
characterizes the goblet cell type.

```{r volcano-plot-goblet, fig.width=4.5, fig.height=4}
goblet_genes <- "Gp2"
...
Indeed, there is a very close correspondence between the topic and
cluster, with several characteristic genes (e.g. *Gp2*) showing
somewhat stronger enrichment in the topic.

```{r z-scores-droplet-G-vs-k1, fig.width=4, fig.height=3}
...
print(p7)
rows <- which(abs(diff_count_clusters_droplet$Z[,"G"] > 2) |
              abs(diff_count_droplet$Z[,1]) > 2)
p7b <- logfoldchange_scatterplot(diff_count_clusters_droplet$beta[rows,"G"],
                                 diff_count_droplet$beta[rows,1],
...
p11 <- volcano_plot_with_highlighted_genes(diff_count_droplet,"k2",
                                           basal_genes,
	 								 	   label_above_quantile = 0.995)
print(p11)
rows <- which(abs(diff_count_clusters_droplet$Z[,"B"] > 2) |
              abs(diff_count_droplet$Z[,2]) > 2)
rows <- is.element(rownames(diff_count_droplet$Z),basal_genes)
