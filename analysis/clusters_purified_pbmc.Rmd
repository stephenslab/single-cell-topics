...
```{r pca-plots-for-paper-1, echo=FALSE}
...
```{r pca-plots-for-paper-2, echo=FALSE}
...
with expression of cell-type-specific genes, such as *CD79A* for
B-cells:
...
```{r loadings-plot-t-cells, fig.width=2.5, fig.height=3.5}
x  <- samples$celltype
...
ggsave("loadings_plot_pbmc_purified.eps",p6,width = 2.5,height = 3.5)
```
...
set.seed(1)
fit1 <- select_loadings(fit,loadings = rows[x == "B"])
fit2 <- select_loadings(fit,loadings = rows[x == "CD14+"])
fit3 <- select_loadings(fit,loadings = rows[x == "CD34+"])
fit4 <- select_loadings(fit,loadings = rows[x == "dendritic"])
fit5 <- select_loadings(fit,loadings = rows[x == "NK"])
fit6 <- select_loadings(fit,loadings = rows[x == "CD8+"])
fit7 <- select_loadings(fit,loadings = rows[x == "T"])
...
ggsave("plots/structure_plot_pbmc_purified.eps",
