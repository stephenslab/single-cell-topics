...
```{r pca-plots-with-expression, width=7.5, height=3}
pca_ggplot_call <- function (dat, pcs, fill.type, fill.label)
  ggplot(dat,aes_string(x = pcs[1],y = pcs[2],color = "y")) +
    ...
    scale_color_gradientn(na.value = "lightskyblue",
                          colors=c("skyblue","gold","darkorange","magenta")) +
    theme_cowplot(font_size = 8) +
    theme(plot.title = element_text(size = 8,face = "plain"))
p5a <- pca_plot(poisson2multinom(fit),
                fill = log10(counts[,"ENSG00000105369"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "CD79A (B-cells)",color = "log10(count)")
p5b <- pca_plot(poisson2multinom(fit),
 	            fill = log10(counts[,"ENSG00000163220"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "S100A9 (CD14+)",color = "log10(count)")
p5c <- pca_plot(poisson2multinom(fit),
	            fill = log10(counts[,"ENSG00000174059"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "CD34 (CD34+)",color = "log10(count)")
p5d <- pca_plot(poisson2multinom(fit),
                fill = log10(counts[,"ENSG00000197992"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "CLEC9A (dendritic)",color = "log10(count)")
p5e <- pca_plot(poisson2multinom(fit),
	            fill=log10(counts[,"ENSG00000105374"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "NKG7 (NK)",color = "log10(count)")
p5f <- pca_plot(poisson2multinom(fit),
	            fill = log10(counts[,"ENSG00000167286"]),
                ggplot_call = pca_ggplot_call) +
       labs(title = "CD3D (T-cells)",color = "log10(count)")
p5g <- pca_plot(poisson2multinom(fit),
	            fill = log10(counts[,"ENSG00000153563"]),
                ggplot_call = pca_ggplot_call) +
