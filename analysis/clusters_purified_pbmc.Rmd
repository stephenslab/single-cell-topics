...
In summary, we have subdivided the cells into 7 subsets. The
...
cluster_colors <- c("skyblue",     # dendritic
                    "dodgerblue",  # B-cells
                    "forestgreen", # CD14+
					"darkmagenta", # CD34+ 
                    "red",         # CD8+
...
```{r clustering-6, fig.width=5, fig.height=3, message=FALSE}
...
p3 <- pca_plot(poisson2multinom(fit),fill = samples$celltype) +
...
p4 <- ggplot(pdat,aes(x = method,y = d)) +
...
p5 <- structure_plot(select(poisson2multinom(fit),loadings = rows),
                     ...
                     topics = topics,colors = topic_colors[topics],
   	  			     perplexity = c(70,30,30,30,30,30,70),
