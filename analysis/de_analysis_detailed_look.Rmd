...
the fastTopics package using simulalted data. We show the benefits of
...
very similar when the cells separate into distinct clusters.
...
A simple MCMC algorithm is used to approximate the posterior means
estimates of the log-fold change statistics and the posterior
intervals. To check that the posterior calculations are reasonably
accurate, one can perform the DE analysis twice, and compare the
outputted $z$-scores by the two `de_analysis` runs.
...
and compare the $z$-scores estimated by these two runs:
...
```{r assess-monte-carlo-4, fig.width=3, fig.height=3}
pdat <- data.frame(x = de1$z[,2],y = de2$z[,2])
