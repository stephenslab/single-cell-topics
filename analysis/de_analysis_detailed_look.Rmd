...
A very popular method for DE analysis in single-cell RNA sequencing
data is DESeq2. Here we apply DESeq2 to these same UMI count data, and
show that the fastTopics DE results are very similar to DESeq2. Since
DESeq2 requires that each cell be assigned to a cluster, we apply
DESeq2 to the subset of cells that were are almost entirely (>99%)
simulated from a single topic; approximatmely 2/3 of the cells were
simulated from one topic, and about the remaining 1/3 of the cells
were simulated from a mixture of the two topics. There may be a small
loss of accuracy in the DESeq2 results since DESeq2 is using only 2/3
of the data, but overall we expect that the fastTopics DE analysis
will closely replicate the DESeq2 analysis.
...
recommended for single-cell RNA-seq data (refer to the DESeq2 vignette
for an explanation of these recommendations). To replicate the
fastTopics analysis as closely as possible, we shrink the LFC
estimates using adaptive shrinkage.
