...
pdat1 <- data.frame(kl = pmin(D[,1],0.00025),de = factor(nonzero_lfc))
pdat2 <- data.frame(z = clamp(de$z[,1],-4,+4),de = factor(nonzero_lfc))
pdat1 <- pdat1[i,]
pdat2 <- pdat2[i,]
...
pdat2 <- create_fdr_vs_power_curve(-de.noshrink$lpval[,1],nonzero_lfc)
pdat3 <- create_fdr_vs_power_curve(-de$lpval[,1],nonzero_lfc)
