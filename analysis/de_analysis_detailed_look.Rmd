...
LFC estimates, or the LFC estimates corresponding to genes with lower
expression, tend to be more strongly shrunk toward zero.
...
plot compare the point estimates (specifically, MLEs) of the LFCs (X
axis) against the shrunken posterior mean estimates (Y axis). What is
striking is that even very large (in magnitude) point estimates can be
shrunk to zero; this happens because although the MLEs can be far from
zero, the HPD intervals are also very large.
...
which doesn't fully account for uncertainty in the LFCs. As a result,
sometimes genes with very large LFCs get highly ranked, but after a
full accounting for uncertainty, they are no longer ranked highly.

```{r etc-1, fig.width=8, fig.height=2.5}
...
To drive home this point, suppose we used the KL-divergences and the
...
```{r etc-3, fig.height, fig.width=4.5, fig.height=3}
...
# z.deseq <- with(results(deseq),log2FoldChange/lfcSE)
```

*Add text here.*

```{r fasttopics-vs-deseq2-zscores, eval=FALSE}
...
```{r fasttopics-vs-deseq2-pvalues, eval=FALSE}
