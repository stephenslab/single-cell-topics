...
LFCs. Without fully accounting for uncertainty, sometimes genes with
very large LFCs get highly ranked. (Note that, since the K-L
divergence is not a signed ranking measure like the $z$-score, here we
focus only on genes that are estimated to have greater expression in
topic 2 compared to topic 1.)
...
pdat  <- rbind(cbind(pdat1,method = "kl"),
               cbind(pdat2,method = "noshrink"),
               cbind(pdat3,method = "shrink"))
p14 <- ggplot(pdat,aes(x = fdr,y = power,color = method)) +
  geom_point(size = 0.3)  +
  scale_color_manual(values = c("royalblue","limegreen","darkorange")) +
  theme_cowplot()
print(p14)
