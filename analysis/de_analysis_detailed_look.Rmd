...
on the *p*-values and *s*-values (which are computed from the LFSRs).
...
Both the *z*-score and *p*-value/*s*-value plots show that the
adaptive shrinkage step more strongly encourages the LFC estimates for
the non-DE genes toward zero.
...
pdat2 <- create_fdr_vs_power_curve(-de.noshrink$lpval[i,2],nonzero_lfc[i],
                                   length.out = 140)
pdat3 <- create_fdr_vs_power_curve(de$lfsr[i,2],nonzero_lfc[i],
                                   length.out = 140)
...
pdat2 <- create_roc_curve(-de.noshrink$lpval[i,2],nonzero_lfc[i],
                          length.out = 100)
pdat3 <- create_roc_curve(de$lfsr[i,2],nonzero_lfc[i],length.out = 100)
pdat  <- rbind(cbind(pdat1,method = "kl"),
               cbind(pdat2,method = "noshrink"),
               cbind(pdat3,method = "shrink"))
...
analysis twice, and compare the posterior mean estimates and
*z*-scores returned by the two `de_analysis` runs. If the posterior
means and *z*-scores are consistent between the two runs, this
provides a good indication that they are mostly accurate.
...
Now we compare the posterior means and *z*-scores estimated by these
two runs in simple scatterplot:

```{r assess-monte-carlo-2, fig.width=6, fig.height=3}
pdat <- data.frame(postmean1 = de1$postmean[,2],
                   postmean2 = de2$postmean[,2],
                   z1        = de1$z[,2],
				   z2        = de2$z[,2])
...
```{r assess-monte-carlo-4, fig.width=6, fig.height=3}
pdat <- data.frame(postmean1 = de1$postmean[,2],
                   postmean2 = de2$postmean[,2],
                   z1        = de1$z[,2],
				   z2        = de2$z[,2])
...
(as implemented in the R package ashr).
...
Note that `lfcShrink` outputs the shrunken log-fold change estimate
the "log2FoldChange" column) as the posterior mean estimated by ashr,
and the s.e. (the "lfcSE") column is the posterior standard deviation
estimmated by ashr.
...
Correspondingly, the *s*-value distributions are very similar in
shape:

```{r fasttopics-vs-deseq2-pvalues, fig.height=2.5, fig.width=8, warning=FALSE}
pdat <- data.frame(deseq2 = deseq$svalue,
                   shrink = de$svalue[,2],
                   de     = factor(nonzero_lfc))
...
we can compare *p*-values against *s*-values:

```{r fasttopics-vs-mast-pvalues, fig.height=3, fig.width=3}
pdat <- data.frame(MAST       = pmax(-6,log10(mast$p_val)),
                   fastTopics = pmax(-6,log10(de$svalue[genes,2])))
ggplot(pdat,aes(x = MAST,y = fastTopics)) +
  geom_point() +
  geom_abline(intercept = 0,slope = 1,color = "skyblue",linetype = "dotted") +
  labs(x = "MAST log10 p-value",y = "fastTopics log10 s-value") +
  theme_cowplot()
```

The *p*-values are *s*-value broadly similar. The effect of the
...
pdat1 <- create_fdr_vs_power_curve(deseq$svalue,nonzero_lfc,length.out = 200)
...
pdat1 <- create_roc_curve(deseq$svalue,nonzero_lfc,length.out = 100)
