...
large differences. (Note that MAST occasionally produces extreme LFC
estimates, so any estimates larger than 6 or less than -6 were shown
as 6 or -6.)
...
Finally, we plot, for each method, FDR vs. power for identifying genes
that are differentially expressed between among the two topics or
groups.

```{r plot-fdr-vs-power, fig.height=2.75, fig.width=4}
...
At low false discovery rates, DESeq2 has a slight advantage over the
other methods, but overall the performance of all three methods is
very close.
