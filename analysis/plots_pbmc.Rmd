...
distinct subcluster, which we label as "A1". Otherwise, there is are
...
Here, we find least three distinct clusters in the projection onto
PCs 3 and 4:

```{r pbmc-68k-clustering-2, fig.width=5, fig.height=4, eval=FALSE}
n <- nrow(pca_68k)
x <- rep("c1",n)
pc3 <- pca_68k[,"PC3"]
pc4 <- pca_68k[,"PC4"]
...
```{r pbmc-68k-clustering-3, eval=FALSE}
table(samples_68k$cluster)
```

**TO DO:** Add text here.

```{r pbmc-purified-structure-plot, fig.width=7, fig.height=2, eval=FALSE}
set.seed(1)
...
```{r pbmc-purified-vs-celltypes, eval=FALSE}
...
```{r pbmc-68k-ternary-plot-c2, fig.width=4, fig.height=4, eval=FALSE}
par(mar = c(0,0,0,0))
