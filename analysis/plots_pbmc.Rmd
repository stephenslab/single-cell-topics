...
fit2 <- select(poisson2multinom(fit_purified),
               loadings = which(samples_purified$cluster == "c3"))
pca <- prcomp(fit2$L)$x
n   <- nrow(fit2$L)
x   <- rep("c3",n)
pc1 <- pca[,1]
pc2 <- pca[,2]
x[pc1 < 0] <- "c3a"
x[pc1 > 0.5 & pc2 < 0.15] <- "c3b"
p5 <- pca_plot_with_labels(fit2,c("PC1","PC2"),factor(x),
                           c("gray","forestgreen","greenyellow")) +
      labs(fill = "cluster")
print(p5)
```
