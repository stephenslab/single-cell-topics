...
be captured by clusters. This is also an example of substructure that
is much better captured by linear representations such as PCA as
opposed to nonlinear dimensionality reduction methods such as *t*-SNE
and UMAP.
...
In summary, we have subdivided these data into 8 subsets:
...
rows <- sort(c(sample(which(samples_68k$cluster == "A1a"),1000),
               sample(which(samples_68k$cluster == "A1b"),800),
...
p17 <- structure_plot(select(poisson2multinom(fit_68k),loadings = rows),
                      ...
                      perplexity = c(100,100,100,70,80,50,50,50),
