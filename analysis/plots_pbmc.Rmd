...
smallest cluster (C) is less than 1% the size of the largest (A1). By
contrast, community detection methods such as the Louvain algorithm
are preferentially biased toward more uniformly sized clusters (this
is a known limitation of community detection methods).

Looking more closely at the top two PCs in cluster B, we identify two
large clusters, with the remaining samples assigned to a "background"
cluster, B3.
...
Cluster A subdivides fairly into two large clusters, labeled A1 and
A2. The remaining in-between samples are assigned to subset A3.
...
```{r pbmc-68k-clustering-6, fig.width=8, fig.height=3}
...
```{r pbmc-68k-clustering-7, fig.width=3.5, fig.height=3}
