...
FACS-purified PBMC data sets. The topics in the 68k fit are reordered
to better align with the other topics.
...
p2 <- pca_plot_with_labels(fit_purified,c("PC4","PC5"),
                           samples_purified$cluster,
                           purified_cluster_colors) + labs(fill = "cluster")
plot_grid(p1,p2)
```

PCs 4 and 5 in mixture of FACS-purified PBMC data:

```{r pbmc-purified-pca-3}
p2 <- pca_plot(poisson2multinom(fit_purified),pcs = 4:5,k = 2)
```
...
```{r pbmc-purified-pca-with-celltypes}
