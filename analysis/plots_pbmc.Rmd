...
Load the $k = 6$ Poisson NMF model fit, and the results of the
differential exxpression analysis and gene-set enrichment analysis.

```{r load-purified-fit}
resultdir <- "../output/pbmc-purified"
fit_purified <- readRDS(file.path(resultdir,
                                  "rds/fit-pbmc-purified-scd-ex-k=6.rds"))$fit
load(file.path(resultdir,"postfit-pbmc-purified-scd-ex-k=6.RData"))
```
...
```{r}
fit_purified_clusters <-
  init_poisson_nmf_from_clustering(counts,samples_purified$cluster)
out <- diff_count_analysis(fit_purified_clusters,counts)
out$colmeans <- out$colmeans[genes$ensembl]
out$pval     <- out$pval[genes$ensembl,]
out$beta     <- out$beta[genes$ensembl,]
out$se       <- out$se[genes$ensembl,]
out$Z        <- out$Z[genes$ensembl,]
diff_count_res_clusters <- out
```
