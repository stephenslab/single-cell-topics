...
pc3 <- pca_68k[,"PC3"]
pc4 <- pca_68k[,"PC4"]
x[pc4 > 0.13 | pc3/1.9 - 0.17 > -pc4] <- "c2"
x[pc4 > 0.75] <- "c3"
...
```{r pbmc-purified-structure-plot, fig.width=7, fig.height=2}
set.seed(1)
...
```{r pbmc-68k-structure-plot, fig.width=7, fig.height=2}
set.seed(1)
...
```{r pbmc-purified-vs-celltypes}
with(samples_purified,table(celltype,cluster))
```

Let's look more closely at cluster 3 in `fit_purified`:

```{r pbmc-purified-ternary-plot-c3, fig.width=4, fig.height=4}
...
par(mar = c(0,0,0,0))
L    <- poisson2multinom(fit_purified)$L
pdat <- cbind(L,samples_purified["celltype"])
rows <- which(samples_purified$cluster == "c3")
levels(pdat$celltype) <- pbmc_purified_celltype_colors
TernaryPlot(alab = "k2",blab = "k5",clab = "k6",
            grid.col = "gainsboro",grid.minor.lines = 0)
TernaryPoints(pdat[rows,c(2,5,6)],pch = 21,cex = 0.65,col = "white",
  			  bg = as.character(pdat[rows,"celltype"]))
```

Now let's look more closely at cluster 1 in `fit_purified`:

```{r pbmc-purified-ternary-plot-c1, fig.width=4, fig.height=4}
par(mar = c(0,0,0,0))
...
```{r pbmc-68k-ternary-plot-c1, fig.width=4, fig.height=4}
par(mar = c(0,0,0,0))
