...
+ Run differential expression analysis using the topic model (cache
  the results).

+ Run the differential expression analysis using the clusters (cache
  these results, too).

+ Create plots comparing t-SNE, UMAP and PCA on the topic proportions.
...
(labeled "CD34+" and "CD14+"). There appear to be at least a couple other
...
The two subclusters, CD34+ and CD14+, account for most of the samples in
cluster C. We also define a third subset, X---a "background
cluster"---containing all the samples that were not assigned to CD34+ or
CD14+.
...
subcluster, which we label as "NK"; the subset of samples that are not
assigned to this cluster are labeled "A2". (The NK subcluster is much
...
```{r pbmc-purified-clustering-7, fig.width=8, fig.height=3}
...
```{r pbmc-purified-clustering-8, fig.width=4, fig.height=3}
pca <- prcomp(fit$L)$x
n   <- nrow(fit$L)
x   <- rep("T",n)
pc1 <- pca[,1]
...
clusters (labeled NK, B, CD34+, CD14+). And, indeed, they align
closely with their inclusion in the individual FACS-purified data
sets:
...
+ Because of their close correspondence, subsequent analysis of topics
  ...
  NK, B, CD34+, CD14+. For example, cluster B corresponds almost
  exactly to the B-cell data set. The largest cluster, cluster A2, is
  mostly comprised of T-cells.
...
+ Many samples labeled as "CD34+" are not assigned to the CD34+
  cluster ("CD34+"). This reflects the fact that this population was much
  less pure (45%) than the others, so we would expect some cells
  labeled as "CD34+" to not necessarily be CD34+ cells.

+ Cluster X is a heterogeneous cluster with a relatively small number
...
B. The remaining samples are assigned to a subset labeled "A3".
...
Topic 3 appears to characterize natural killer cells, and topic 4 has
yet to be characterized, but may represent some subset of naive
T-cells. This is an example with interesting substructure that can't
be captured well by clusters. Nonetheless, here we (somewhat
arbitrarily) designate two subsets, NK and A1c, in which all samples
in these subsets are mostly explained by topic 3 and 4, respectively:

```{r pbmc-68k-clustering-8, fig.width=4, fig.height=3}
fit  <- select(poisson2multinom(fit_68k),loadings = rows)
pca  <- prcomp(fit$L)$x
n    <- nrow(pca)
x    <- rep("A1a",n)
...
+ As in the purified fit, here we identify a B-cells cluster (B) and
  ...
  cells (C), CD14+ monocytes (CD14+) and dendritic cells (D).
