...
Perform a second differential expression analysis after merging the
two T cell topics:

```{r diff-count-analysis-topics, cache=TRUE}
fit_merged <- merge_topics(fit_no_dendritic,c(1,6))
diff_count_merged <- diff_count_analysis(fit_merged,counts[rows,])
```

Perform another differential expression analysis on the T cells only:
...
```{r volcano-plots-3, fig.width=7, fig.height=8, message=FALSE}
...
p18 <- volcano_plot(diff_count_merged,"k1+k6",genes$symbol,
                    label_above_quantile = 0.999,
 				    subsample_below_quantile = 0.5,
				    filter_low_counts = 5e-5) +
  guides(fill = "none") +
  ggtitle("topics 1 + 6 (T cells)")
p19 <- volcano_plot(diff_count_t,"k1",genes$symbol,
                    label_above_quantile = 0.999,
 				    subsample_below_quantile = 0.5,
				    filter_low_counts = 5e-5) +
  guides(fill = "none") +
  ggtitle("topic 1 (CD4+/CD8+)")
p20 <- volcano_plot(diff_count_topics,"k6",genes$symbol,
                    label_above_quantile = 0.999,
 				    subsample_below_quantile = 0.5,
				    filter_low_counts = 5e-5) +
  guides(fill = "none") +
  ggtitle("topic 6")
plot_grid(p14,p15,p16,p17,p18,p19,p20,nrow = 3,ncol = 3)
```
