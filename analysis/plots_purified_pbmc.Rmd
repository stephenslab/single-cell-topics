...
x <- genes$symbol
x[which(diff_count_topics$beta[,"k3"] < 2)] <- ""
pp14 <- volcano_plot(diff_count_topics,"k3",x,
                     label_above_quantile = 0.998,
 				     subsample_below_quantile = 0.5,
				     filter_low_counts = 5e-5) +
  ggtitle("topic 3 (B cells)")
...
```{r volcano-plots-5, results="hide", eval=FALSE}
...
The interactive volcano plots mayalso be viewed by clicking these links:
...
```{r ribosomal-proteins-topic-6, fig.width=4, fig.height=2, message=FALSE, eval=FALSE}
...
i <- which(celltype == "CD19+ B")
j <- which(is.element(genes$symbol,rpgenes))
pdat <- data.frame(x = fit$L[i,6],y = rowSums(counts[i,j])/rowSums(counts[i,]))
p20 <- ggplot(pdat,aes(x = x,y = y)) +
  geom_point() +
  geom_smooth(method = "lm",se = FALSE,color = "dodgerblue",size = 0.5,
              linetype = "dashed") +
  ylim(0,0.6) +
...
i <- which(!(celltype == "CD19+ B" | celltype == "CD34+"))
pdat <- data.frame(x = fit$L[i,6],y = rowSums(counts[i,j])/rowSums(counts[i,]))
p21 <- ggplot(pdat,aes(x = x,y = y)) +
  geom_point() +
  geom_smooth(method = "lm",se = FALSE,color = "dodgerblue",size = 0.5,
              linetype = "dashed") +
  ylim(0,0.6) +
  labs(x = "topic 6 proportion",
       y = "ribosomal expression",
	   title = "all except B and CD34+ cells") + 
  theme_cowplot(font_size = 10) +
  theme(plot.title = element_text(size = 10,face = "plain"))
plot_grid(p20,p21)
```

The list of ribosomal protein genes is from
[Yoshihama *et al* (2002)][yoshihama-2002].
