...
List of ribosomal protein genes is from doi:10.1101/gr.214202.

```{r ribosomal-proteins-topic, fig.width=3, fig.height=3, message=FALSE, eval=FALSE}
...
plot(fit$L[i,6],rowSums(counts[i,j])/rowSums(counts[i,]),pch = 20)
i <- which(!(celltype == "CD19+ B" | celltype == "CD34+"))
x <- fit$L[i,6]
y <- rowSums(counts[i,j])/rowSums(counts[i,])
plot(x,y,pch = 20,xlab = "topic 6 proportion",
     ylab = "prop. of expression from ribosomal genes",
	 main = sprintf("cor = %0.2f",cor(x,y)))
```
