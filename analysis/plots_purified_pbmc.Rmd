...
diff_count_topics_2 <- diff_count_analysis(fit_no_dendritic,counts[rows,],
                                           pseudocount = 100,numiter = 100)
```
...
x <- genes$symbol
x[with(diff_count_topics,
       beta[,"k3"] < 2 |
	   Z[,"k3"] < 9 |
	   40*beta[,"k3"] + 2*Z[,"k3"] < 400)] <- ""
pp14 <- volcano_plot(diff_count_topics,"k3",x,betamax = 15,
                     label_above_quantile = 0.5,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 3 (B cells)")
x <- genes$symbol
x[which(diff_count_topics$beta[,"k2"] < 2)] <- ""
pp15 <- volcano_plot(diff_count_topics,"k2",x,
                     label_above_quantile = 0.998,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 2 (CD14+ cells)")
x <- genes$symbol
x[which(diff_count_topics$beta[,"k5"] < 2)] <- ""
pp16 <- volcano_plot(diff_count_topics,"k5",x,
                     label_above_quantile = 0.998,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 5 (CD34+ cells)")
x <- genes$symbol
x[which(diff_count_topics$beta[,"k4"] < 2)] <- ""
pp17 <- volcano_plot(diff_count_topics,"k4",x,
                     label_above_quantile = 0.998,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 4 (NK cells)")
x <- genes$symbol
x[which(diff_count_topics$beta[,"k1"] < 5)] <- ""
pp18 <- volcano_plot(diff_count_topics,"k1",x,
                     label_above_quantile = 0.997,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 1 (T cells)")
x <- genes$symbol
x[which(diff_count_topics$beta[,"k6"] < 0)] <- ""
pp19 <- volcano_plot(diff_count_topics,"k6",x,
                     label_above_quantile = 0.998,
 				     subsample_below_quantile = 0.5) +
  ggtitle("topic 6 (ribosomal proteins)")
...
```{r volcano-plots-5, results="hide"}
