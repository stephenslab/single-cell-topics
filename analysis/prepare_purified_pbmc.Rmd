...
These are the steps taken to prepare the mixture of FACS-purified
PBMC data for topic modeling analysis. These data are the "reference
transcriptome" profiles generated via single-cell RNA-seq of 10
bead-enriched subpopulations as described in
[Zheng *et al* (2017)][zheng-2017].
...
Download the "Gene/cell matrix (filtered)" tar.gz file from the
...
Cytotoxic T cells and CD8+/CD45RA+ Naive Cytotoxic T Cells. Unpack the
individual tar archives and re-organize the files into 10 data
subdirectories, each containing three (compressed) files,
`barcodes.tsv.gz`, `genes.tsv.gz` and `matrix.mtx.gz`. The "datasets"
variable lists the names of all subdirectories used, along with the
