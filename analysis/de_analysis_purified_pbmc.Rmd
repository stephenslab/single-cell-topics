...
```{r mcmc-accuracy-plots-for-paper, echo=FALSE}
...
each closely correspond to a topic (topic 2 and topic 3). Here we
focus on genes for which the *z*-score is greater than 2 in at least
one of the analyses. Genes are colored according to the LFSR estimated
in the fastTopics analysis.

```{r bcells-z-scores, fig.height=3.2, fig.width=8, warning=FALSE}
i <- "CD19+ B"
k <- "k3"
...
```{r scatterplots-plots-for-paper, echo=FALSE}
...
+ [topic 1 (T cells)](de_analysis_purified_pbmc_tcells.html)

+ [topic 2 (CD14+ monocytes)](de_analysis_purified_pbmc_cd14.html)

+ [topic 3 (B cells)](de_analysis_purified_pbmc_bcells.html)

+ [topic 4 (natural killer cells)](de_analysis_purified_pbmc_nkcells.html)

+ [topic 5 (CD34+)](de_analysis_purified_pbmc_cd34.html)

+ [topic 6 (ribosomal protein genes)](de_analysis_purified_pbmc_ribosome.html)
