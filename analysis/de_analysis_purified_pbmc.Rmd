...
Next we visualize the structure inferred by the $K = 6$ topic model
...
Comparing the distributions of all *z*-scores for B cells and CD14+
cells, we see that the DE analysis allowing for grades of membership
has many more *z*-scores near zero, yet still allows for large
*z*-scores, illustrating the flexibility of the model. (Because there
are a few extremely large and extremely small *z*-scores, for better
visualization z-scores larger than 20 in magnitude are counted as 20
or -20.)
...
```{r bcells-z-scores, fig.height=3.5, fig.width=8, warning=FALSE}
i <- "CD19+ B"
k <- "k3"
...
p1 <- ggplot(pdat1,aes(x = postmean.deseq,y = postmean.fasttopics,
  ...
  scale_fill_gradient2(low = "royalblue",mid = "lightskyblue",high = "red",
                       midpoint = -4.5) +
  xlim(-12.3,13.1) +
  ylim(-10,13.1) +
...
p2 <- ggplot(pdat2,aes(x = postmean.deseq,y = postmean.fasttopics,
  ...
  geom_text_repel(color = "darkgray",size = 2.25,fontface = "italic",
                  segment.color = "darkgray",segment.size = 0.25,
                  min.segment.length = 0,max.overlaps = Inf,na.rm = TRUE) +
  scale_fill_gradient2(low = "royalblue",mid = "lightskyblue",
                       high = "red",midpoint = -4.5) +
  xlim(-8.2,15) +
  ylim(-8.2,15) +
  labs(x = "DESeq2",y = "fastTopics",
