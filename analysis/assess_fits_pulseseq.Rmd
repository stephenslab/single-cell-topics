---
title: Assess Poisson NMF fits in pulse-seq data
...
Load the results of running `fit_poisson_nmf` on the pulse-seq data,
...
p1 <- plot_loglik_vs_rank(fits) +
  ...
  scale_y_continuous(breaks = seq(0,6e7,2e7)) +
  labs(x = "number of topics (K)") +
  theme_cowplot(font_size = 12)
p2 <- create_progress_plots(dat,fits,"loglik")
p3 <- create_progress_plots(dat,fits,"res")
ggsave("loglik_vs_k_pulseseq.eps",p1,height = 3,width = 3.25)
ggsave("progress_loglik_pulseseq.eps",p2,height = 6,width = 9)
ggsave("progress_res_pulseseq.eps",p3,height = 6,width = 9)
```
