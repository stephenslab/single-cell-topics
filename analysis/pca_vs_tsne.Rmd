...
Next, run PCA on the mixture proportions for this random subset of 2,000
samples.
...
Run *t*-SNE on the mixture proportions.
...
Then run UMAP on the mixture proportions.
...
By the projection of the samples onto the first two PCs, the B-cells
cluster is distinct from the others, whereas the CD14+ and CD34+ cells
do not separate as well.
...
ggsave("../plots/pca_vs_tsne.eps",plot_grid(p1,p2,p3,nrow = 1),
       height = 2,width = 8)
```

Note that the first 2 PCs should be sufficient for capturing the full
structure in the mixture proportions as they explain >96% of the
variance:
