...
p11  <- pca_plot(fit2,pcs = 1:2,fill = "none")
print(p11)
```
...
rows <- which(x == "D")
fit2 <- select(poisson2multinom(fit),loadings = rows)
p12  <- pca_plot(fit2,pcs = 1:2,fill = "none")
p13  <- pca_hexbin_plot(fit2,pcs = 1:2) + guides(fill = "none")
plot_grid(p12,p13)
```
...
```{r, eval=FALSE}
p16 <- pca_plot(poisson2multinom(fit),pcs = 5:6,fill = log10(counts[,"Chga"]))
p17 <- pca_plot(poisson2multinom(fit),pcs = 5:6,fill = log10(counts[,"Gnat3"]))
```

In summary, we have subdivided the pulse-seq data into 7 subsets,
which includes a background cluster (U).

```{r clustering-10, fig.width=8, fig.height=1.75, message=FALSE}
...
cluster_colors  <- c("royalblue",   # B
					 ...
					 "darkmagenta", # I
                     "gainsboro")   # U
p18 <- pca_plot(poisson2multinom(fit),pcs = 1:2,fill = samples$cluster) +
       scale_fill_manual(values = cluster_colors) +
       labs(fill = "cluster")
p19 <- pca_plot(poisson2multinom(fit),pcs = 3:4,fill = samples$cluster) +
       scale_fill_manual(values = cluster_colors) +
       labs(fill = "cluster")
p20 <- pca_plot(poisson2multinom(fit),pcs = 5:6,fill = samples$cluster) +
       scale_fill_manual(values = cluster_colors) +
       labs(fill = "cluster")
plot_grid(p18,p19,p20,nrow = 1)
```
...
p21 <- structure_plot(select(poisson2multinom(fit),loadings = rows),
...
rows <- which(x == "C")
fit2 <- select(poisson2multinom(fit),loadings = rows)
p22  <- pca_plot(fit2,pcs = 1:2,k = c(5,8,10))
print(p22)
```
...
p23 <- pca_plot(fit2,pcs = 3:4,k = c(4,5,6))
print(p23)
```
...
p24  <- pca_plot(fit2,pcs = 2:3,fill = log10(counts[rows,"Krt13"])) +
