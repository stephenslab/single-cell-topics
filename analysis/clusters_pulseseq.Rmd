...
The structure is more evident from "hexbin" plots showing the density
of the points example, two clusters in PCs 1 and 2 emerge in the
hexbin plot:
...
From these PCA plots, we define 4 clusters, labeled A, C, Cil and
...
We can refine cluster A somewhat by plotting PCs computed from cluster
A only:

```{r clustering-4, fig.width=3, fig.height=2.5}
rows <- which(x == "A")
...
```{r clustering-6, fig.width=6, fig.height=2.5}
rows <- which(x == "C")
fit2 <- select(poisson2multinom(fit),loadings = rows)
...
at all distinct, it may nonetheless be useful to subdivide these
points (somewhat arbitrarily) into two subsets, labeled B and C.
...
explore this additional structure in a subsequent analysis.
