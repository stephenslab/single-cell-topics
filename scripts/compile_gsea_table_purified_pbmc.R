# A short script use to compile the susie GSEA results for the
# purified PBMC data into a table.
library(Matrix)
library(susieR)
library(pathways)
source("../code/gsea.R")
...
rows <- match(rownames(X),gene_info$Ensembl)
rownames(X) <- gene_info[rows,"Symbol"]
rownames(Y) <- gene_info[rows,"Symbol"]
...
Y      <- Y[,topics]
gsea   <- gsea[topics]
colnames(Y) <- paste0("k",1:6)
names(gsea) <- paste0("k",1:6)
